...
if exps[0] <= today < exps[1]:
    ...
    year_var = year[-1]
    #building the ticker
    current_exp = "NQ" + exps_symbol[1] + year_var
    next_exp = "NQ" + exps_symbol[2] + year_var
    ...
    year_var = year[-1]
    #building the ticker
    current_exp = "NQ" + exps_symbol[2] + year_var
    next_exp = "NQ" + exps_symbol[3] + year_var
    exps_to_check.append(current_exp)
    exps_to_check.append(next_exp)
    print("Current exp:", current_exp)
    print("Next exp:", next_exp)
    ...
    year_var1 = year1[-1]
    #building the ticker
    current_exp = "NQ" + exps_symbol[3] + year_var
    next_exp = "NQ" + exps_symbol[0] + year_var1
    exps_to_check.append(current_exp)
    exps_to_check.append(next_exp)
    print("Current exp:", current_exp)
    print("Next exp:", next_exp)
    ...
    year_var1 = year1[-1]
    #building the ticker
    current_exp = "NQ" + exps_symbol[0] + year_var1
    next_exp = "NQ" + exps_symbol[1] + year_var1
    exps_to_check.append(current_exp)
    exps_to_check.append(next_exp)
    print("Current exp:", current_exp)
    print("Next exp:", next_exp)
    ...
    year_var = year[-1]
    #building the ticker
    current_exp = "NQ" + exps_symbol[0] + year_var
    next_exp = "NQ" + exps_symbol[1] + year_var
    exps_to_check.append(current_exp)
    exps_to_check.append(next_exp)
    print("Current exp:", current_exp)
    print("Next exp:", next_exp)
...
while contratto is None:
    ...
    for e in exps_to_check:
        ...
        app = Expirations()
        print(e)
        app.connect("127.0.0.1", socket, 1)
...
def time_shift_period_check():
    ...
    us_switch1 = datetime.date(US1Y, US1M, US1D)
    it_switch1 = datetime.date(EU1Y, EU1M, EU1D) #max

    us_switch2 = datetime.date(US2Y, US2M, US2D) #max
    it_switch2 = datetime.date(EU2Y, EU2M, EU2D)
    ### ----------------------------------------------------- <<<
...
class Volume(EWrapper, EClient):
    ...
    def historicalData(self, reqId, bar):
        ...
        conn = sqlite3.connect('NQdatabase.db') #<<<--- be sure is the correct DB
        cur = conn.cursor()
...
if str(today) != str(yesterday):
    ...
    conn = sqlite3.connect('NQdatabase.db')
    cur = conn.cursor()
...
def retrive_joint_data(days, store):
    ...
    print('\n')
    print('EXPIRATION INFO NQ (for joint) ---')
    ...
    class Volume(EWrapper, EClient):
        ...
        def historicalData(self, reqId, bar):
            ...
            hist_data_NQ[date] = {'volume' : volume, 'volatility' : 0}
    ...
    class Volatility(EWrapper, EClient):
        ...
        def historicalData(self, reqId, bar):
            ...
            hist_data_NQ[date]['volatility'] = range
    ...
    if exps[0] <= today < exps[1]:
        ...
        year_var = year[-1]
        #building the ticker
        current_exp = "MNQ" + exps_symbol[1] + year_var
        next_exp = "MNQ" + exps_symbol[2] + year_var
        exps_to_check.append(current_exp)
        exps_to_check.append(next_exp)
        print("Current exp:", current_exp)
        print("Next exp:", next_exp)
        ...
        year_var = year[-1]
        #building the ticker
        current_exp = "MNQ" + exps_symbol[2] + year_var
        next_exp = "MNQ" + exps_symbol[3] + year_var
        exps_to_check.append(current_exp)
        exps_to_check.append(next_exp)
        print("Current exp:", current_exp)
        print("Next exp:", next_exp)
        ...
        year_var1 = year1[-1]
        #building the ticker
        current_exp = "MNQ" + exps_symbol[3] + year_var
        next_exp = "MNQ" + exps_symbol[0] + year_var1
        exps_to_check.append(current_exp)
        exps_to_check.append(next_exp)
        print("Current exp:", current_exp)
        print("Next exp:", next_exp)
        ...
        year_var1 = year1[-1]
        #building the ticker
        current_exp = "MNQ" + exps_symbol[0] + year_var1
        next_exp = "MNQ" + exps_symbol[1] + year_var1
        exps_to_check.append(current_exp)
        exps_to_check.append(next_exp)
        print("Current exp:", current_exp)
        print("Next exp:", next_exp)
        ...
        year_var = year[-1]
        #building the ticker
        current_exp = "MNQ" + exps_symbol[0] + year_var
        next_exp = "MNQ" + exps_symbol[1] + year_var
        exps_to_check.append(current_exp)
        exps_to_check.append(next_exp)
        print("Current exp:", current_exp)
        print("Next exp:", next_exp)
    ...
    class Volume(EWrapper, EClient):
        ...
        def historicalData(self, reqId, bar):
            ...
            hist_data_MNQ[date] = {'volume' : volume, 'volatility' : 0}
    ...
    class Volatility(EWrapper, EClient):
        ...
        def historicalData(self, reqId, bar):
            ...
            hist_data_MNQ[date]['volatility'] = range
    ...
    print('\n')
    print('NQ data:')
    for i in hist_data_NQ:
        print (i, hist_data_NQ[i])

    print('\n')
    print('MNQ data:')
    for i in hist_data_MNQ:
        print (i, hist_data_MNQ[i])

    print('\n')

    for i in hist_data_NQ:
        temp_volatility = list()
        temp_volatility.append(hist_data_NQ.get(i, {}).get('volatility'))
        temp_volatility.append(hist_data_MNQ.get(i, {}).get('volatility'))
        joint_hist_data[i]['volatility'] = nearest_quarter(Average(temp_volatility))
        joint_hist_data[i]['volume'] = hist_data_NQ.get(i, {}).get('volume') + hist_data_MNQ.get(i, {}).get('volume')
    ...
    for i in joint_hist_data:
        volume_leak = round((hist_data_MNQ.get(i, {}).get('volume')/joint_hist_data.get(i, {}).get('volume'))*100)
...
def M1_calc():
    ...
    # # adaptive MM module <<<---- WARNING!: changing this, change even the reverse and MOM parameters
    if M1_calc.M1 <= 0:
        print("Error - M1 <= 0")
    elif 0 < M1_calc.M1 < 12:
        mm = 4
        session_mm = mm
    elif 12 <= M1_calc.M1 < 15:
        mm = 5 # <<<<<----- it should be 4 normally, but 5 with a partially adaptive MM
        session_mm = mm
    elif M1_calc.M1 >= 15:
        mm = 5
        session_mm = mm
...
if rest is False:
    if M1 >= 21 and STP_operativity is True:
        continuation_mode = True
        print('--- STP ORDERS ALLOWED ---')

continuation_margin = 2.25
...
def calibrate_security():
    ...
    #     gap_allowed = nearest_quarter(4.75 + (4.75 * volume_leak_avg)) #previous 3
    #     print('High risk profile - Gap allowed:', gap_allowed)
    # elif volatility_expected == 'Medium':
    #     gap_allowed = nearest_quarter(3.50 + (3.50 * volume_leak_avg)) #previous 2.75
    #     print('Medium risk profile - Gap allowed:', gap_allowed)
    # elif volatility_expected == 'Low':
    #     gap_allowed = nearest_quarter(2.25 + (2.25 * volume_leak_avg))
...
class TestApp(EWrapper, EClient):
            ...
            if start_op_time <= now.time() <= end_time:
                    ...
                    if done is False and go is True and automatic is True:
                        if difference > gap_allowed  or difference < -gap_allowed: ### <<<------------ pay attention
                            ...
                            send_message(25169251, ('NQ - Too much volatility. Security on'))
        ...
        while True:
            ...
            if done is True and go is False: ### TLM
                ...
                if executed is True and is_short is True and op_done is False: #add the op_done variable
                    ...
                    # position manager
                    elif current_price == tp or current_price == (tp + 0.25) or current_price == (tp + 0.50) or current_price == (tp + 0.75) or current_price == (tp + 1) or current_price == (tp + 1.25) or current_price == (tp + 1.5) or current_price == (tp + 1.75) or current_price == (tp + 2) and position_manager_ready is False and position_manager is True and automatic is True and stp_moved is False:
                    ...
                    #         executed = False
                        ...
                        is_short = False
                        op_done = True
                        send_message(25169251, ('NQ - Managed'))
                        print('time:', now)
                        time.sleep(1)
                        if op_reset_allowed is True:
                            self.cancelOrder(self.nextValidOrderId-1)
                ...
                #emergency close positions <<<----------
                    ...
                    if automatic is True:
                        ...
                        is_short = False
                        executed = False
                        send_message(25169251, ('NQ - Emergency close'))
    ...
    #             status = 'OFF'
    #             myButton.config(text=f'NQ AUTO MODE {status}')
    ...
    #         status = 'OFF'
    #
    #     myButton = Button(root, text=f'NQ AUTO MODE {status}', padx=200, pady=50, command=switch, font=("Futura", 21))
    #
    #     if automatic is True:
    #         myButton.config(bg='#55FF33')
    #     else:
    ...
    def operativity(self):
        ...
        def round_down(x):
            return float(math.floor(x / 5.0)) * 5

        def round_down_STP(x):
        ...
        def order_analysis(up_trend, dwn_trend, order1):
            ...
            if continuation_mode is False:
                if Mega_Area_distance == 2.5: #EVALUATE IF MOVE IT UP TO 5. in that case, change even Mega_Area_distance default value
        ...
        def isPA(number):
            return number % 5 == 0 #TRUE if is a PA
        ...
        while True:
            ...
            if (notif_start1 < now.time() < notif_end1 and notification is True) or ((start_time < now.time() < end_time) and starting_up is True):
                ...
                # M1 recalibrating ------------->>> Recalibrate rounding like on ES but with NQ parameters
                # if range <= 42:
                #     if M1 > 12:
                #         M1 = 12
                ...
                # if range > avg_volatility_10d: #previous > 87.5
                #     if M1 < 20 and reverse == default_reverse:
                #         dynamic_M1 = False
                #         # M1 = 20
                #         # print('M1 recalibrated. New M1:', M1)
                #         M1_original = M1
                ...
                if range > (avg_volatility_10d * 1.35): #above 87.5
                    #BETA
                    ...
                    # print("Potential gain: ", gain)
                    # print("Potential loss: ", loss)
                elif (avg_volatility_10d * 0.65) <= range <= (avg_volatility_10d * 1.35): #between 42 and 87.5
                    #BETA
                    continuation_mode = False
                    print('--- LMT ORDERS ALLOWED ---')
                    ...
                    # # M1 = float(avg_pom)
                    # #BETA ^^^
                    if (M1 > nearest_quarter(float(avg_pom)*1.33333)) or (M1 < nearest_quarter(float(avg_pom)*0.8)):
                        M1 = float(avg_pom)
                        mm = 5
                        session_mm = mm
                    # # mm = 5
                    # session_mm = mm
                    # # trigger_price_delta = 2.25
                    # pl = int(p_and_l(mm)) #profit and loss
                    # gain = "+" + str(pl)
                    # loss = "-" + str(pl)
                    # print("Potential gain: ", gain)
                    # print("Potential loss: ", loss)
                elif range < (avg_volatility_10d * 0.65): #below 42
                    #BETA
                    continuation_mode = False
                    print('--- LMT ORDERS ALLOWED ---')
                    volatility_expected = 'Low'
                    # # M1 = nearest_quarter(float(avg_pom) * 0.8) # 13 or 15
                    # #BETA ^^^
                    if M1 > nearest_quarter(float(avg_pom)*0.8):
                        M1 = nearest_quarter(float(avg_pom)*0.8)
                        mm = 4
                        session_mm = mm
                    # # mm = 4
                    # session_mm = mm
                    # # trigger_price_delta = 1.5
                    # pl = int(p_and_l(mm)) #profit and loss
                    # gain = "+" + str(pl)
                    # loss = "-" + str(pl)
                    # print("Potential gain: ", gain)
                ...
                if VP_on is True:
                    try:
                        ...
                        df = yf.download(tickers='NQ=F', start=sd, end=ed, interval="1m") #NQ=F for Nasdaq
            ...
            #EVENING
                ...
                if range > (avg_volatility_10d * 1.35): #above 87.5
                    ...
                    # # M1 = float(avg_ser)
                    # #BETA ^^^
                    if (M1 > nearest_quarter(float(avg_ser)*1.33333)) or (M1 < nearest_quarter(float(avg_ser)*0.8)):
                        M1 = float(avg_ser)
                        mm = 5
                        session_mm = mm
                    # # mm = 5
                    ...
                    print('--- LMT ORDERS ALLOWED ---')
                    volatility_expected = 'Low'
                    # # M1 = nearest_quarter(float(avg_ser) * 0.8) # 13 or 15
                    # #BETA ^^^
                    if M1 > nearest_quarter(float(avg_ser)*0.8):
                        M1 = nearest_quarter(float(avg_ser)*0.8)
                        mm = 4
                        session_mm = mm
                    # # mm = 4
                    # session_mm = mm
            ...
            if datetime.time(sedici, 31, 0) < now.time() < datetime.time(sedici, 32, 0):
                ...
                for m in movements:
                    try:
                        cur.execute("INSERT INTO f_pom_movements VALUES (NULL, ?, ?, ?, ?)", (todayDB, m, range_bars_ticks, contratto)) #### database disk image is malformed risk
                    except:
                ...
                # -------------------- ADJUSTING MAX MOVEMENT
                if len(movements) > 0:
                    index = movements.index(max_move)
                    #print("Max movement index: ", index)
                    while movements[-1] != max_move and movements[-2] != max_move and movements[index+1] < mm: #original 3.50
                        index = movements.index(max_move)
                        if movements[index+1] < mm: # <<<<<----- WARNING! look at the MM
            ...
            if datetime.time(ventuno, 46, 0) < now.time() < datetime.time(ventuno, 47, 0):
                ...
                if len(movements) > 0:
                    ...
                    while movements[-1] != max_move and movements[-2] != max_move and movements[index+1] < mm: #original 3.50
                        index = movements.index(max_move)
                        if movements[index+1] < mm: #original 3.50
                            max_move = max_move + (movements[index+2] - movements[index+1])
                            del movements[index+1]
                            del movements[index+1]
                            movements[index] = max_move
            ...
            ### FIVE MINUTE RESET
            if start_trading_time <= now.time() <= end_trading_time and five_min_done is True and five_min_reset is False:
                # op_reset()
    ...
    # UP TREND ----------
                if up_trend is True:
                    ...
                    if start_op_time <= now.time() < end_time and done is False:
                        ...
                        # if fm_check_done is False:
                            ...
                            if continuation_mode is True and STP_operativity is True:
                                temp_order = (round_up_STP(last_min + M1)) + continuation_margin
                            else:
                                ####### IF YOU CHANGE SOMETHING HERE, CHANGE EVEN INSIDE THE TREND
                                if 0 < M1 < 15:
                                    temp_order = round_up(last_min + M1) # to place the order to the closest psycho area <<<----------
                                elif M1 >= 15:
                            ...
                            # if 0 < M1 < 12:
                            #     temp_order = myround(last_min + M1)
                            # elif M1 == 12:
                            #     temp_order = round_up(last_min + M1)
                            # elif 12 < M1 < 15:
                            #     temp_order = myround(last_min + M1)
                            ...
                            if temp_move >= M1 or current_price >= temp_order or temp_order in price_levels:
                                ...
                                if first_move_notification is True and STP_operativity is False:
                                    ...
                                    # send_message(25169251, ('NQ - First move > M1, or above the order, or order touched'))
                                if op_reset_allowed is True:
                                    fm_check_done = False
                                    done = False
                            elif temp_move < M1:
                    ...
                    if now.time() > start_op_time and go is True:
                        ...
                        if continuation_mode is True and STP_operativity is True:
                            order = (round_up_STP(last_min + M1)) + continuation_margin
                        else:
                            # ADAPTIVE ORDERS
                            if order_moved is False:
                                if 0 < M1 < 15:
                                    order = round_up(last_min + M1) # to place the order to the closest psycho area <<<---------- WARNING HERE
                                elif M1 >= 15:
                                    order = round_up(last_min + M1) # to place the order to the next psycho area <<<----------


                        # if 0 < M1 < 12:
                        #     order = myround(last_min + M1)
                        # elif M1 == 12:
                        ...
                        if current_price < order and executed is False and start_op_time <= now.time() <= end_time:
                            if order != last_order and order > -1:
                                ...
                                if reverse == default_reverse:
                                    ...
                                    print('Virtual Range Bars: 20 ticks')
                                ...
                                if trigger_price <= current_price < order and trigger_price > -1:
                                    ...
                                    if order_placed is False and automatic is True and green_light is True:
                                        if continuation_mode is True and STP_operativity is True:
                                            ...
                                            # if (volatility_expected == 'Medium' or volatility_expected == 'High') and current_price < (p_close - 5): ### BETA 26 OTT 2020
                            ...
                            if start_time < now.time() < start_trading_time:
                                ...
                                if STP_operativity is False:
                                    # winsound.PlaySound("sounds/voice/Allison/five_minutes.wav", winsound.SND_ASYNC)
                                    # send_message(25169251, ('NQ - Triggered in the first 5 minutes'))
                                    print('...')
                            if start_trading_time <= now.time() <= end_trading_time and news_range is False:
                                ...
                                if order_placed is True:
                                    send_message(25169251, ('NQ - Order executed'))
                                #position direction recognizer
                                #for close_position function
                                # is_short = True
                        elif now.time() > end_time:
                            ...
                            go = False
                            send_message(25169251, ('NQ - No trade'))
                    ...
                    if rev_move > -reverse: # <<<<<----- WARNING! look at the MM
                        if current_price > last_max:
                ...
                if dwn_trend is True:
                    ...
                    if start_op_time <= now.time() < end_time and done is False:
                        ...
                        # if fm_check_done is False:
                            ...
                            if continuation_mode is True and STP_operativity is True:
                                temp_order = (round_down_STP(last_max - M1)) - continuation_margin
                            else:
                                ####### IF YOU CHANGE SOMETHING HERE, CHANGE EVEN INSIDE THE TREND
                                if 0 < M1 < 15:
                                    temp_order = round_down(last_max - M1) # to place the order to the closest psycho area <<<----------
                                elif M1 >= 15:
                            ...
                            # if 0 < M1 < 12:
                            #     temp_order = myround(last_max - M1)
                            # elif M1 == 12:
                            #     temp_order = round_down(last_max - M1)
                            # elif 12 < M1 < 15:
                            #     temp_order = myround(last_max - M1)
                            ...
                            if temp_move >= M1 or current_price <= temp_order or temp_order in price_levels:
                                ...
                                if first_move_notification is True and STP_operativity is False:
                                    ...
                                    print("First move: ", temp_move)
                                    first_move_notification = False
                                    # send_message(25169251, ('NQ - First move > M1, or below the order, or order touched'))
                                if op_reset_allowed is True:
                                    fm_check_done = False
                                    done = False
                            elif temp_move < M1:
                    ...
                    if now.time() > start_op_time and go is True:
                        ...
                        if continuation_mode is True and STP_operativity is True:
                            order = (round_down_STP(last_max - M1)) - continuation_margin
                        else:
                            # ADAPTIVE ORDERS
                            if order_moved is False:
                                if 0 < M1 < 15:
                                    order = round_down(last_max - M1) # to place the order to the closest psycho area <<<---------- WARNING HERE
                                elif M1 >= 15:
                                    order = round_down(last_max - M1) # to place the order to the next psycho area <<<----------


                        # if 0 < M1 < 12:
                        #     order = myround(last_max - M1)
                        # elif M1 == 12:
